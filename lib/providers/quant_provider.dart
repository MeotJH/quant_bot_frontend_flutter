...
final trendFollowProvider = AsyncNotifierProvider.autoDispose.family<TrendFollowNotifier, TrendFollowModel, String>(
  TrendFollowNotifier.new,
);

class TrendFollowNotifier extends AutoDisposeFamilyAsyncNotifier<TrendFollowModel, String> {
  @override
  Future<TrendFollowModel> build(String ticker) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return TrendFollowModel(firstLineChart: [], secondLineChart: [], models: [], recentStockOne: QuantStockModel());
      }

      final List stockHistory = response.data['stock_history'];
      final Map<String, dynamic> stockInfo = response.data['stock_info'];

      List<QuantModel> models = stockHistory.map((e) => QuantModel.fromJson(stock: e)).toList();
      final QuantStockModel quantStockModel = QuantStockModel.fromJson(json: stockInfo);

      return TrendFollowService(models: models, recentStockOne: quantStockModel).generateTrendFollows();
    } catch (e) {
      return TrendFollowModel(firstLineChart: [], secondLineChart: [], models: [], recentStockOne: QuantStockModel());
    }
  }
