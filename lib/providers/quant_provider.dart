  ...
  Future<TrendFollowModel> build(String ticker) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        return TrendFollowModel(firstLineChart: [], secondLineChart: []);
      }

      final List data = response.data['trend_follows'];
      List<QuantModel> models = data.map((e) => QuantModel.fromJson(stock: e)).toList();

      return TrendFollowService(models: models).generateTrendFollows();
    } catch (e) {
      return TrendFollowModel(firstLineChart: [], secondLineChart: []);
    }
  }
